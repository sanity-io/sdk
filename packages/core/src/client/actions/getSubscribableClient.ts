 ...
 * @public
 */
export const getSubscribableClient = createResourceAction(clientStore, ({instance, state}) => {
  return (options: ClientOptions): Subscribable<SanityClient> => {
    ...
    const client$ = state.observable.pipe(
      ...
      // as we add more things that can change with how clients are configured,
