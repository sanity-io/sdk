 ...
 * exchange already in progress). A callback that fails validation while a
 * session is already `LOGGED_IN` (e.g. a remount before the params were
 * stripped) is ignored with a warning and still returns the cleaned URL, so a
 * string result does not imply an exchange ran.
 *
 * @public
 */
