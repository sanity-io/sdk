...
import {CorsOriginError} from '../utils/CorsOriginError'
import * as storage from '../utils/storage'
import {createBroadcastChannel} from './createBroadcasetChannel'
import {getSessionId} from './sessionId'
import {type AuthState, type AuthStore, type AuthStoreOptions, type LoginMethod} from './types'
 ...
 * @internal
 */
export function _createAuthStore({projectId, dataset}: AuthStoreOptions): AuthStore {
  const loginMethod: LoginMethod = 'dual'
  const apiHost = undefined
  const clientFactoryOption = undefined
  ...
  // TODO: deal with login component at the react level. Just export anything needed here.
  // const LoginComponent = createLoginComponent({
  //   ...providerOptions,
  //   getClient: () => state$.pipe(map((state: AuthState) => state.client)),
  //   loginMethod,
  // })
...
): loginMethod is 'dual' | 'token' {
  const cookielessCompatibleLoginMethods = ['dual', 'token']
  return cookielessCompatibleLoginMethods.includes(loginMethod)
}
...
export const createAuthStore = _createAuthStore
