import {type DocumentHandle, type SanityConfig} from '../config/sanityConfig'
...
export function createActionBinder(instanceKeyFn: KeyFn<unknown[]>, storeKeyFn?: KeyFn<unknown[]>) {
  ...
  return function bindAction<TState, TParams extends unknown[], TReturn>(
    ...
    return function boundAction(instance: SanityInstance, ...params: TParams) {
      ...
      if (!storeInstance) {
        storeInstance = createStoreInstance(instance, {...storeDefinition, name: storeRegistryKey}, ...params)
        console.log('creating store instance', storeRegistryKey)
...
export const bindActionByDatasetAndDocumentHandle = createActionBinder(
  createProjectDatasetKey,
  (config, documentHandle) => {
    const {projectId, dataset} = config
    const initialKey = `${projectId}.${dataset}`
    const additionalParams: string[] = []
    const {perspective: perspectiveFromDocumentHandle} = documentHandle as DocumentHandle
