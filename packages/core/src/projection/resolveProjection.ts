...
import {bindActionByDatasetAndDocumentHandle} from '../store/createActionBinder'
 ...
 * @beta
 */
const _resolveProjection = bindActionByDatasetAndDocumentHandle(
  projectionStore,
  (
    ...
    firstValueFrom(
      getProjectionState<Record<string, unknown>>(instance, options).observable.pipe(
        filter((state): state is ProjectionValuePending<Record<string, unknown>> => {
          return !!state?.data
        }),
      ),
    ),
