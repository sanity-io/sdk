import {createSanityInstance, getDefaultProjectId, type SanityInstance} from '@sanity/sdk'
...
describe('createStateSourceHook', () => {
  ...
  it('should create a hook that provides access to state source', () => {
    const mockInstance = createSanityInstance({sources: {default: {projectId: 'p', dataset: 'd'}}})
  ...
  it('should recreate state source when instance changes', () => {
    const mockInstance1 = createSanityInstance({
      sources: {default: {projectId: 'p1', dataset: 'd'}},
    })
    const mockInstance2 = createSanityInstance({
      sources: {default: {projectId: 'p2', dataset: 'd'}},
    })
    ...
    const stateSourceFactory = vi.fn((instance: SanityInstance) => ({
      subscribe: vi.fn(),
      getCurrent: () => getDefaultProjectId(instance.config),
      observable: throwError(() => new Error('unexpected usage of observable')),
    }))
