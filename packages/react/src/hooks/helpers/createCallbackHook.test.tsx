...
describe('createCallbackHook', () => {
  ...
  it('should create a hook that provides a memoized callback', () => {
    ...
    const {result, rerender} = renderHook(() => useTestHook(), {
      wrapper: ({children}) => (
        <ResourceProvider defaultResource={{projectId: 'p', dataset: 'd'}} fallback={null}>
          {children}
        </ResourceProvider>
      ),
    })
  ...
  it('should create new callback when instance changes', () => {
    ...
    const {result, unmount} = renderHook(() => useTestHook(), {
      wrapper: ({children}) => (
        <ResourceProvider defaultResource={{projectId: 'p1', dataset: 'd'}} fallback={null}>
          {children}
        </ResourceProvider>
      ),
    })
    ...
    const {result: result2} = renderHook(() => useTestHook(), {
      wrapper: ({children}) => (
        <ResourceProvider defaultResource={{projectId: 'p2', dataset: 'd'}} fallback={null}>
          {children}
        </ResourceProvider>
      ),
    })
