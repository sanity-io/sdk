...
import {useDocumentInteractionHistory} from './useDocumentInteractionHistory'
...
describe('useDocumentInteractionHistory', () => {
  ...
  it('should initialize with correct connection status', () => {
    const {result} = renderHook(() => useDocumentInteractionHistory(mockDocumentHandle))
  ...
  it('should send correct message when recording events', () => {
    ...
    expect(node.post).toHaveBeenCalledWith('HISTORY_UPDATE', {
      version: '1',
      eventType: 'viewed',
      documentId: 'mock-id',
