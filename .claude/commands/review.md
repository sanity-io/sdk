...
- Renovate release coverage — a runtime dependency added to `packages/core` or `packages/react` must be matched by a production rule in `.github/renovate.json` so Renovate's `postUpgradeTasks` hook writes a patch changeset when it bumps. `@sanity/*` deps and inline (non-catalog) `dependencies` are covered automatically. The gap is a **catalog, non-`@sanity`** runtime dep (its catalog depType hides it from the catch-all) — that matches no production rule, gets no changeset, and silently misses its patch release, so flag it (it needs a name-matched production rule). There is no CI guard for this
