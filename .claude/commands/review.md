...
- Renovate release coverage — a runtime dependency added to `packages/core` or `packages/react` must be matched by a changeset-writing rule in `.github/renovate.json` so Renovate's `postUpgradeTasks` hook writes a patch changeset when it bumps. An **inline (non-catalog)** `dependencies` entry is covered automatically by the file-scoped rule. A **catalog** runtime dep (its catalog depType hides it from any file path) matches only the explicit name list — so a new `catalog:` runtime dep must be added to `matchPackageNames` on that rule, or it gets no changeset and silently misses its patch release. Flag it. There is no CI guard for this
